...
const AddAgentModal = ({
  ...
  const toggleAll = () =>
    setSelectedUrls((prev) => {
      const next =
        prev.size === sitemapUrls.length ? new Set() : new Set(sitemapUrls);
      if (typeof window !== "undefined") {
        localStorage.setItem("selectedSitemapUrls", JSON.stringify([...next]));
      }
      return next;
...
const filterCompanyPages = (urls) => {
  ...
  const handleWebsiteBlur = async () => {
    const raw = (form.businessUrl || "").trim();
    if (!raw) return;
    ...
    try {
      ...
      const isValid =
        result?.valid === true ||
        String(result?.valid).toLowerCase() === "true";

      setIsWebsiteValid(isValid);
      setForm((p) => ({ ...p, businessUrl: url }));
  ...
  const handleNext = async () => {
    if (step === 1) {
      ...
      try {
        ...
        Swal.fire(
          "Error",
          "Something went wrong while checking agent status.",
          "error"
        );
      }
    }
    ...
    if (step === 4 && !form.language && !form.voice)
      return alert("language and voice are required");
  ...
  // const handleUrlVerification = async (rawUrl) => {
  //   console.log(rawUrl,"rawurl")
  //   try {
  //     let url = (rawUrl || "").trim();
  //     if (!url) return;
  //     const data = await validateWebsite(url);
  //     const isValid =
  //       data?.valid === true || String(data?.valid).toLowerCase() === "true";
  ...
  //       try {
  //         const res = await listSiteMap(url);
  //         console.log("response of list url", res);
  //       } catch (e) {
  //         console.error("listSiteMap failed:", e);
  //       }
  //     } else {
  //       console.warn("Validation failed:", data?.reason ?? "Unknown reason");
  //     }
  //   } catch (err) {
  //     console.error("handleUrlVerification error:", err);
  //   }
  // };

  // const handleUrlVerify = (url) => {
  //   if (url.trim()) {
  //     handleUrlVerification(url);
  //   }
  // };
        ...
        {step === 3 && (
          <div className="space-y-4 border rounded-xl p-4 shadow">
            ...
            <label className="inline-flex items-center gap-2 text-sm text-gray-700">
              ...
              />
              <span>I don’t have Google Business</span>
            </label>
            ...
            <div className="relative w-full">
              ...
              {/* VIEW button + dropdown (only when we have URLs) */}
                ...
                <div className="mt-3">
                  <div className="relative inline-block">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-full bg-violet-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-violet-700"
                    >
                      View
                    </button>

                    {showSitemap && (
                      <div className="absolute z-50 mt-2 w-[420px] overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-black/5">
                        ...
                        <div className="flex items-center justify-between bg-violet-600 px-4 py-3 text-white">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              checked={
                                selectedUrls.size > 0 &&
                                selectedUrls.size === sitemapUrls.length
                              }
                              indeterminate={
                                selectedUrls.size > 0 &&
                                selectedUrls.size < sitemapUrls.length
            ...
            <label className="inline-flex items-center gap-2 text-sm text-gray-700">
              ...
              />
              <span>I don’t have Website URL</span>
            </label>
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (
                  form.about &&
                  (containsSpecialChars(form.about) ||
                    containsEmoji(form.about))
                ) {
                  alert(
                ...
                if (hasGoogle) {
                  const query = encodeURIComponent(
                    `${form.businessName || "N/A"} ${form.address || "N/A"}`
                  );
                ...
                const mergedUrls = [];
                const rawUrl = sessionStorage.getItem("businessUrl") || "";
                ...
                const uniqueMerged = [...new Set(mergedUrls)];

                console.log(uniqueMerged, "mergedUrls");
                ...
                try {
                  ...
                  //   );
                  // }
                  let prevKb=localStorage.getItem('knowledgeBaseId')
                  if(prevKb !== null &&
                    prevKb !== undefined &&
                    prevKb !== "null" &&
                    prevKb !== "undefined" &&
                    ...
                    try {
                      await axios.delete(
                        `https://api.retellai.com/delete-knowledge-base/${prevKb}`,
                        {
                          headers: {
                            Authorization: `Bearer ${process.env.NEXT_PUBLIC_RETELL_API}`,
                      ...
                      console.log('an error occured while delete prev kb')
                    }
                  }
                  ...
                  if (knowledgeBaseId) {
                    ...
                    // );
                    try {
                      const sessionSelectedSiteMapUrls = JSON.parse(localStorage.getItem("selectedSitemapUrls"))
                      const response = await axios.patch(
                        `${process.env.NEXT_PUBLIC_API_URL}/api/businessDetails/updateKnowledeBase/${businessId}`,
                        {
                          knowledge_base_name: knowledgeBaseName,
                          scrapedUrls: JSON.stringify(sessionSelectedSiteMapUrls) || [],
            ...
            <Select
              onValueChange={(v) =>
                setForm({
                  ...
                  language: v,
                  agentLanguage:
                    languages?.find((lang) => lang?.locale == v)?.name || v,
                })
              }
              value={form.language}
            >
