  ...
  export default function ManualNotification({ notification, onSuccess }: ManualNotificationProps) {
    ...
    const [selectedType, setSelectedType] = useState<"notification" | "email" | "both">(
      notification?.sendType as any || "notification"
    );
    const [isSendLater, setIsSendLater] = useState(!!notification?.scheduledDate);
    type AudienceOption = "all" | "free" | "paid" | "paysGo" | string; // string for plan types

    const [availablePlans, setAvailablePlans] = useState<string[]>(['Starter','Scaler','Growth','Corporate']);
    // const [audience, setAudience] = useState<AudienceOption>(notification?.targetPlatform|| "all");
    const [audience, setAudience] = useState<AudienceOption>(availablePlans.includes(notification?.targetPlatform || "")?'plan' :notification?.targetPlatform || "all");

    const [selectedPlan, setSelectedPlan] = useState<string | null>(availablePlans.includes(notification?.targetPlatform || "")? notification?.targetPlatform: null);
    ...
    useEffect(() => {
      if (notification) {
        ...
        setSelectedType(notification.sendType as "notification" | "email" | "both");
      ...
      <div className="p-4 space-y-4">
        ...
        <Card className="w-full">
          <CardContent className="space-y-4 pt-6">
            ...
            <UISelect value={audience} onValueChange={setAudience} disabled={isLoading}>
              ...
              <SelectContent>
                ...
                <SelectItem value="payG">Pay as you Go Enabled Users</SelectItem>
                <SelectItem value="plan">Specific Plan Type</SelectItem>
              </SelectContent>
            </UISelect>
          </div>
            ...
            {audience === "plan" && (
              <div className="space-y-2">
                ...
                <UISelect value={selectedPlan || ""} onValueChange={setSelectedPlan} disabled={isLoading}>
                  <SelectTrigger className="w-full">
            ...
            <div className="flex gap-4">
             {!notification?.id && <Button onClick={handleSendNowClick} disabled={isLoading || !title || !description}>
