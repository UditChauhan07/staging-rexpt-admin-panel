  ...
  const fetchTickets = async () => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/tickets/get_tickets_by_admin`,
        {
          headers: {
      ...
      <Card className="overflow-x-auto">
          ...
          <tbody>
            {currentTickets.map((ticket) => (
              <TableRow key={ticket?.ticketId}>
                ...
                <TableCell>
                  {ticket?.user?.isUserType === 2 ||
                  ticket?.user?.isUserType === 3
                    ? "Partner"
                    : "User"}
                </TableCell>
        ...
        <div className="flex items-center justify-between mt-6">
          <div className="text-sm text-gray-600">
            ...
            {Math.min(indexOfLastTicket, tickets.length)} of {tickets.length}{" "}
            tickets
          </div>
