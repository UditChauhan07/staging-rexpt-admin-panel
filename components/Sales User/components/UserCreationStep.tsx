...
// const UserCreationStep: React.FC<UserCreationStepProps> = ({ data, onUpdate, onNext }) => {
//   const [formData, setFormData] = useState<FormData["user"]>(
//     data.user || { name: "", email: "", phone: "", role: "", referalName: "", password: "" }
//   );
//   const [errors, setErrors] = useState<{ [key: string]: string }>({});
...
//       }
//     }
//     if (["2", "3", "4"].includes(formData.role) && !formData.password?.trim()) {
//       newErrors.password = "Password is required";
//     } else if (formData.password) {
//       const strongPasswordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\w\s]).{8,}$/;
//       if (!strongPasswordRegex.test(formData.password)) {
...
//   return (
//     <StepWrapper step={1} totalSteps={4} title="User Creation" description="Create a new user account.">
//       <form onSubmit={handleNext} className="space-y-6">
//         <div className="space-y-4">
//           <div className="space-y-2">
//             <Label htmlFor="name">Full Name <span className="text-red-500">*</span></Label>
...
//         </div>
//         <div className="flex flex-col sm:flex-row gap-3 pt-4 justify-end">
//           <Button type="submit" className="flex-1 bg-purple-600 hover:bg-purple-700 disabled:opacity-50" disabled={emailExists || loading}>
//             {loading ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <ChevronRight className="w-4 h-4 ml-2" />}
//             Next: Business Details
//           </Button>
//         </div>
